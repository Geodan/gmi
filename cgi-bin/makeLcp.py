...
SELECT AddRasterConstraints('model_wildfire', 'ahn1_%(runid)s'::name, 'rast'::name);
	...
	SELECT AddRasterConstraints('model_wildfire', 'emptyraster_%(runid)s'::name, 'rast'::name);
		...
		#FUEL
		sys.stdout.flush()
		subprocess.call(self.gdal_translate_path + " --config GDAL_FORCE_CACHING true -of AAIGrid -a_nodata -999 -outsize 100% 100% \"PG:host="+settings.pgserver_host+" port="+settings.pgserver_port+" dbname=models user=postgres password=flups schema=model_wildfire table=fuelmodel_"+str(runid)+"\" "+ self.output_path + "/fuelmodel.asc", shell=True)      
		#ELEV
		...
		#EMPTY RASTER
		subprocess.call(self.gdal_translate_path + " --config GDAL_FORCE_CACHING true -of AAIGrid -a_nodata -999 -outsize 100% 100% \"PG:host="+settings.pgserver_host+" port="+settings.pgserver_port+" dbname=models user=postgres password=flups schema=model_wildfire table=emptyraster_"+str(runid)+"\" " + self.output_path + "/emptyraster.asc", shell=True)
		#Some unneeded steps
